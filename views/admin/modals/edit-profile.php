<div class="w-full z-1 h-screen fixed top-0 left-0 frosted hidden" id="editProfileModal">
    <div  data-type="modal" class="absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 margin-auto w-3/4 bg-white rounded-lg shadow-lg p-10">
        ...
        <form method="POST" id="formUpdateProfile" action="/register" enctype='multipart/form-data'>
        <input type="hidden" name="id">
        <input type="hidden" name="form_submit" value="true">
        <input type="hidden" name="edit_profile" value="true">
        <div class="flex space-x-1 py-2 absolute top-10 right-10">
            <div class="inline-block flex">
                <input type="hidden" name="section" value="settings">
                <input id="logo-upload" type="file" name="imgs[]" class="sr-only">
                <label for="logo-upload" class="text-indigo-700 mt-4 align-middle rounded py-1 px-4 hover:text-indigo-900 font-semibold">Change Profile</label>
            </div>
        ...
        <div class="w-full py-4">
            <button type="button" class="updateProfileBtn text-indigo-600 float-right font-bold rounded py-2 px-4 bg-indigo-200 hover:text-indigo-900 hover:bg-indigo-200">Save</button>
        </div>
        </form>
    </div>
</div>